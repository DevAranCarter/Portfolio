import { ReactNode } from 'react';
import {
  Box,
  Flex,
  Avatar,
  Link,
  Button,
  Menu,
  MenuButton,
  MenuList,
  MenuItem,
  MenuDivider,
  useDisclosure,
  useColorModeValue,
  Stack,
  useColorMode,
  Center,
} from '@chakra-ui/react';
import { MoonIcon, SunIcon } from '@chakra-ui/icons';

const NavLink = ({ children }: { children: ReactNode }) => (
  <Link
    px={2}
    py={1}
    rounded={'md'}
    _hover={{
      textDecoration: 'none',
      bg: useColorModeValue('gray.200', 'gray.700'),
    }}
    href={'#'}>
    {children}
  </Link>
);

export default function Nav() {
  const { colorMode, toggleColorMode } = useColorMode();
  const { isOpen, onOpen, onClose } = useDisclosure();
  return (
    <>
      <Box bg={useColorModeValue('gray.300', 'gray.400')} px={4}>
        <Flex h={16} alignItems={'center'} justifyContent={'space-between'}>
          <Box>Devin Carter
          </Box>
          <Flex alignItems={'center'}>
            <Stack direction={'row'} spacing={60}>
              <Button _hover={{backgroundColor:'gray.100'}} backgroundColor={'white'}> About me</Button>
              <Button _hover={{backgroundColor:'gray.100'}} backgroundColor={'white'}> Projects</Button>
              <Button _hover={{backgroundColor:'gray.100'}} backgroundColor={'white'}>  Contact</Button>
              

            </Stack>
          </Flex>

          <Flex alignItems={'center'}>
            <Stack direction={'row'}>
              
            <Link href='https://github.com/DevAranCarter'><Avatar size={'sm'} src="https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png">
          </Avatar> 
              </Link>
            <Link href='https://www.linkedin.com/in/devin-carter2/'>
              <Avatar size={'sm'} src="data:image/png;base64,">
          </Avatar>
              </Link>
            </Stack>
          </Flex>
        </Flex>
      </Box>
    </>
  );
}